  ...
  <div class="bg-white rounded-3xl shadow-md hover:shadow-2xl border border-gray-100 overflow-hidden flex flex-col transition-all duration-500 hover:-translate-y-2 group">
    ...
    <div class="p-8 pb-4 flex flex-col items-center text-center">
        <div class="relative mb-6">
            ...
            <img v-if="item.photo" class="w-28 h-28 rounded-full object-cover border-4 border-white shadow-xl relative z-10" :src="item.photo" :alt="item.full_name">
            <div v-else class="w-28 h-28 rounded-full bg-gradient-to-br from-gray-50 to-gray-100 border-4 border-white shadow-xl flex items-center justify-center text-gray-300 text-4xl relative z-10">
                <i :class="item.type === 'unit' ? 'fas fa-building' : 'fas fa-user'"></i>
            </div>
        </div>
    ...
    <div class="px-6 pb-8 pt-2 mt-auto">
        <div class="space-y-3">
            ...
            <template v-if="filteredLhkpns.length > 0">
              <a v-for="lhkpn in filteredLhkpns" :key="lhkpn.id" :href="getLhkpnUrl(lhkpn)" target="_blank" 
                  ...
                  <div class="flex flex-col items-end">
                      <span v-if="lhkpn.full_name" class="text-[9px] font-medium opacity-70 group-hover/link:text-white line-clamp-1 max-w-[120px]">{{ lhkpn.full_name }}</span>
...
const getLhkpnUrl = (lhkpn) => {
  return `${import.meta.env.VITE_API_URL.replace('/api/v1', '')}/lhkpn/view/${lhkpn.id}`
}
