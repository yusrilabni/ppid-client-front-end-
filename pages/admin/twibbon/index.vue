  ...
  <div class="p-6">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
      <div v-for="item in twibbons" :key="item.id" class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden group hover:shadow-md transition flex flex-col">
        <NuxtLink :to="item.status === 'public' ? `/twibbon/${item.slug}` : `/admin/twibbon/${item.slug}`" class="block h-48 bg-gray-100 relative overflow-hidden flex items-center justify-center p-4 checkerboard cursor-pointer" title="Buka Editor Twibbon">
