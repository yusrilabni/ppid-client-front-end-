<template>
  <div class="relative min-h-screen bg-gray-50 pt-4 md:pt-6 pb-16">
    ...
    <div class="container max-w-5xl mx-auto px-4 relative z-10">
      ...
      <div v-if="error" class="mb-6 bg-red-50/90 border border-red-200 text-red-700 px-6 py-4 rounded-2xl shadow-sm flex items-start backdrop-blur-sm relative z-20">
          ...
          <form @submit.prevent="submitForm" enctype="multipart/form-data">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div class="md:col-span-2">
                ...
                <div class="flex space-x-2">
                  <input v-model="form.judul" type="text" id="judul" placeholder="Masukkan judul yang deskriptif..."
                    class="flex-1 px-5 py-4 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all duration-300 font-medium text-gray-800 placeholder-gray-400 shadow-sm" required>
              ...
              <div class="md:col-span-2">
                ...
                <textarea v-model="form.deskripsi" id="deskripsi" rows="3" placeholder="Tuliskan keterangan singkat mengenai dokumen ini..."
                  class="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all duration-300 font-medium text-gray-800 placeholder-gray-400 shadow-sm"></textarea>
              </div>
                ...
                <div class="relative w-full">
                  <button type="button" @click="showJenisDropdown = !showJenisDropdown"
                    ...
                    <span class="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none">
                      <div class="p-1 rounded-lg bg-gray-50 group-hover:bg-blue-50 transition-colors duration-300">
                        <i class="fas fa-chevron-down h-4 w-4 text-gray-400 group-hover:text-blue-500 transition-transform duration-300" :class="showJenisDropdown ? 'rotate-180' : ''"></i>
                      </div>
                    </span>
                  </button>
                  <div v-show="showJenisDropdown" class="absolute mt-1 w-full rounded-2xl bg-white shadow-[0_25px_60px_-15px_rgba(0,0,0,0.3)] z-[9999] border border-gray-100 max-h-72 overflow-y-auto">
                    <label v-for="(opt, idx) in jenisDokumenOptions" :key="idx" 
                           class="flex items-center mx-2 my-1 px-4 py-3 hover:bg-blue-50 cursor-pointer rounded-xl transition-colors"
                  ...
                  <div>
                    ...
                    <div class="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-4">
                      <label class="flex-1 flex items-center p-3 border border-gray-200 rounded-xl bg-white cursor-pointer hover:border-blue-300 transition-all">
                        <input type="radio" v-model="form.visibility" value="public" class="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500">
                      ...
                      <label class="flex-1 flex items-center p-3 border border-gray-200 rounded-xl bg-white cursor-pointer hover:border-blue-300 transition-all">
                        <input type="radio" v-model="form.visibility" value="private" class="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500">
              ...
              <div class="md:col-span-2 border-2 border-dashed border-blue-200 rounded-2xl p-6 bg-blue-50/30 hover:bg-blue-50/60 transition-colors duration-300 group">
                ...
                <div class="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-6 mb-6">
                  <label class="flex items-center cursor-pointer p-4 border border-blue-100 rounded-xl bg-white shadow-sm hover:shadow-md hover:border-blue-300 transition-all">
                    <input type="radio" v-model="form.upload_method" value="file" class="w-5 h-5 text-blue-600 border-gray-300 focus:ring-blue-500">
                    <div class="ml-3">
                  ...
                  <label class="flex items-center cursor-pointer p-4 border border-blue-100 rounded-xl bg-white shadow-sm hover:shadow-md hover:border-blue-300 transition-all">
                    <input type="radio" v-model="form.upload_method" value="link" class="w-5 h-5 text-blue-600 border-gray-300 focus:ring-blue-500">
                    <div class="ml-3">
            ...
            <div class="mt-10 pt-6 border-t border-gray-100 flex flex-col-reverse sm:flex-row justify-end sm:space-x-4 relative z-10">
              ...
              <button type="submit" :disabled="submitting"
                ...
                <span>{{ submitting ? 'Menyimpan...' : 'Publikasikan Dokumen' }}</span>
              </button>
            </div>
          </form>
...
const breadcrumbItems = [
  ...
  { title: 'Tambah Dokumen', url: null, icon: 'fas fa-plus-circle' }
]
...
watch(() => form.value.visibility, (newVal) => {
  if (newVal === 'private' && !isRevertingVisibility) {
  ...
  try {
    ...
    if (form.value.upload_method === 'file') {
      if (fileInput.value && fileInput.value.files[0]) {
        ...
        throw new Error('Silakan pilih file dari perangkat.')
      }
    } else {
      if (!form.value.link) {
    ...
    const res = await api.post('/informasi-pemkab-crud', formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    })
    
    if (res.data.success || res.status === 200 || res.status === 201) {
      router.push(`/transparansi/informasi-pemkab?message=${encodeURIComponent('Dokumen berhasil ditambahkan')}`)
    }
  } catch (err) {
