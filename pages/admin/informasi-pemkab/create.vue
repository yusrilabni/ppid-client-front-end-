...
const breadcrumbItems = computed(() => {
  ...
  if (selectedCategory.value) {
    items.push({ title: selectedCategory.value, url: `/transparansi/informasi-pemkab/${categorySlug.value}`, icon: 'fas fa-file-alt' })
  }
...
const checkSimilarity = async () => {
  ...
  try {
    ...
    // Fallback to /v2/admin/proses-cek-judul if /informasi-pemkab-crud/check-similarity 404s
    let res;
    try {
      res = await api.post("/informasi-pemkab-crud/check-similarity", payload)
    } catch (apiErr) {
...
const submitForm = async () => {
  ...
  try {
    ...
    if (res.data.success || res.status === 200 || res.status === 201) {
      ...
      setTimeout(() => {
        router.push(categorySlug.value ? `/transparansi/informasi-pemkab/${categorySlug.value}?message=${encodeURIComponent('Informasi berhasil ditambahkan.')}` : '/admin')
      }, 1500)
    }
  } catch (err) {
