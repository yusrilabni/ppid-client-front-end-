              ...
              <div class="md:col-span-2">
                ...
                <div class="flex space-x-4">
                  <label class="inline-flex items-center">
                    <input type="radio" v-model="form.status" value="published" class="form-radio h-4 w-4 text-blue-600">
                    <span class="ml-2 text-gray-700">Berlaku</span>
                  </label>
                  <label class="inline-flex items-center">
                    <input type="radio" v-model="form.status" value="draft" class="form-radio h-4 w-4 text-blue-600">
...
const breadcrumbItems = computed(() => {
  ...
  if (form.value.category) {
    items.push({ title: form.value.category, url: `/transparansi/informasi-pemkab/${categorySlug.value}`, icon: 'fas fa-file-alt' })
  }
...
const fetchInformasi = async () => {
  ...
  loadError.value = ''
  try {
    const res = await api.get(`/informasi-pemkab-crud/${informasiId}/edit`)
    if (res.data?.success && res.data.data) {
      const data = res.data.data
            form.value.title = data.judul || ''
      form.value.doc_desc = data.deskripsi || ''
      form.value.category = data.kategori || ''
      form.value.jenis_dokumen = data.jenis_dokumen ? data.jenis_dokumen.split(', ') : []
      form.value.tahun = data.published_at ? data.published_at.substring(0, 4) : (data.tahun || '')
  ...
  try {
    ...
    if (form.value.file_type === 'upload') {
      ...
      if (form.value.file_url) {
        formData.append('link', form.value.file_url)
      }
    }

    const res = await api.post(`/informasi-pemkab-crud/${informasiId}`, formData, {
      headers: {
    ...
    if (res.data?.success || res.status === 200) {
      ...
      setTimeout(() => {
        router.push(categorySlug.value ? `/transparansi/informasi-pemkab/${categorySlug.value}?message=${encodeURIComponent(success.value)}` : '/admin')
      }, 1500)
    }
  } catch (err) {
